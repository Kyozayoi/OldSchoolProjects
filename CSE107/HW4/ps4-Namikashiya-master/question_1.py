...
H: {0,1}^k x D -> {0,1}^l via the CBC construction, as follows:
"""
...
"""
[20 points] Show that H is not collision resistant by presenting an O(T_E + l)
...
def A(K):
    ...
    E0 = E(K, "\x00" * l_bytes)
    E1 = E(K, "\xFF" * l_bytes)
    M1 = "\x00" * l_bytes + E0 + E0
    M2 = "\xFF" * l_bytes + E1 + E0
